		...
		return this.each(function() {
			...
			if ($(self).attr('data-styled') === undefined) {
				...
				var filename = $('<input type="text" readonly="readonly">')
					...
					.addClass('NFI-filename NFI NFI-filename-empty '+guid);
				var wrapper = $("<div>")
					.css({
						'overflow': 'hidden',
						'position': 'relative',
        ...
        filename.val("Прикрепить файл");
				...
				$(self).on("change", function() {
                                	...
                                	if (settings.fullPath) {
                                		filename.val(fullPath).removeClass("NFI-filename-empty");
                                    filename.parents(".form-item").find(".error-wrapper").remove();
                                	} else {
                                		var pathArray = fullPath.split(/[/\\]/);
                                		filename.val(pathArray[pathArray.length - 1]).removeClass("NFI-filename-empty");
                                    filename.parents(".form-item").find(".error-wrapper").remove();
                                	}
                                });
